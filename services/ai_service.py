...
async def parse_reminder_time(text: str, user_timezone: str = "UTC") -> Optional[str]:
    ...
    system_prompt = (
        ...
        "Day abbreviations: mon=Monday, tue=Tuesday, wed=Wednesday, thu=Thursday, fri=Friday, sat=Saturday, sun=Sunday.\n"
        "Rules:\n"
        "- \"tomorrow at HH:MM\" -> next calendar day at that time\n"
        "- A weekday name or abbreviation (e.g. \"tue 14:00\", \"Monday\", \"fri\") -> "
        "find the NEXT occurrence of that weekday AFTER the current moment. "
        "If today IS that weekday but the specified time has already passed, return NEXT WEEK's occurrence (7 days later). "
        "If today IS that weekday and the time has NOT yet passed, return today at that time.\n"
        "- \"in X hours\" -> current datetime + X hours\n"
